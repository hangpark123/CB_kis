...
def _classify(text: str) -> str:
    t = text or ""
    if "리픽싱" in t or ("전환가" in t and "조정" in t): return "REFIX"
    if "전환청구" in t: return "CONVERSION"
    if "조기상환" in t or "풋옵션" in t: return "REDEMPTION"
    if "발행결정" in t or "발행" in t: return "ISSUE"
    return "OTHER"

def _to_utc(d: Optional[dt.datetime]) -> Optional[dt.datetime]:
    if d is None: return None
    if d.tzinfo is None: return d.replace(tzinfo=UTC)
...
def _load_choices():
    ...
    if _CHOICES is None or _LAST_LOAD is None or (now - _LAST_LOAD).total_seconds() > 600:
...
async def _fetch_naver_once(queries: Iterable[str], display: int = 30, mode: str = "all"):
    """
    mode: 'cb' | 'all'
      - 'cb': COMBINED 키워드 포함된 기사만
      - 'all': 질의 결과 전체
    """
    ...
    out.sort(key=lambda x: _iso_to_utc(x["time"]) or dt.datetime.min.replace(tzinfo=UTC), reverse=True)
    return out

@router.get("/news")
async def live_news(q: Optional[str] = None, display: int = 30, minutes: int = 60, mode: str = "auto"):
    """
    온디맨드 뉴스 조회.
    mode: 'auto'|'cb'|'all'
      - auto: q 있으면 'all', 없으면 'cb'
    """
    queries = [s.strip() for s in (q or ",".join(settings.NAVER_NEWS_QUERIES)).split(",") if s.strip()]
...
async def news_stream(
    ...
    use_mode = ("all" if (mode == "auto" and q) else ("cb" if mode == "auto" else mode))
    seen: Set[str] = set()
    def _cutoff(): return dt.datetime.now(UTC) - dt.timedelta(minutes=minutes)
        ...
        try:
            ...
            while True:
                ...
                for r in rows:
                    ...
                    seen.add(key)
                    t = _iso_to_utc(r.get("time"))
                    if t is not None and t < co: continue
...
def _parse_rcept_dt(s: Optional[str]) -> Optional[dt.datetime]:
    if not s: return None
    try:
        return dt.datetime.strptime(s, "%Y%m%d%H%M%S").replace(tzinfo=KST)  # aware (KST)
    except Exception:
        return None

async def _fetch_dart_once(minutes: int = 60, page_count: int = 100, max_pages: int = 3):
    """
    최근 minutes 범위를 커버하도록 DART list.json을 호출.
    - 여러 페이지(page_no=1..max_pages) 순회
    - 여기서는 '모든 공시'를 긁고, scope/시간필터는 호출부에서 적용
    """
...
@router.get("/dart")
async def live_dart(minutes: int = 60, page_count: int = 100, limit: int = 10, scope: str = "cb"):
    """
    최근 공시 조회.
      - minutes: 최근 N분
      - limit: 최대 n건 (기본 10)
      - scope: 'cb'(CB 관련만) | 'all'(모든 공시)
    """
...
async def dart_stream(request: Request, interval: int = 8, minutes: int = 60,
                      page_count: int = 100, scope: str = "cb"):
    """
    DART 공시 SSE 스트림.
      - scope: 'cb' | 'all'
    """
        ...
        try:
            ...
            while True:
                ...
                for r in rows:
                    key = f"{r.get('url')}|{r.get('time')}"
                    if key in seen: continue
                    if scope != "all" and not re.search(COMBINED, r.get("headline") or "", flags=re.I):
                        continue
                    t = _iso_to_utc(r.get("time"))
                    if t is not None and t < co: continue
