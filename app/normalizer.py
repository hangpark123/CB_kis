...
def normalize_recent(minutes=180):
    ...
    with SessionLocal() as s:
        raws = s.execute(select(RawEvent).where(RawEvent.inserted_at >= cutoff)).scalars().all()
        for r in raws:
            ...
            ne = NormEvent(
                stock_code=code, corp_name_kr=corp, event_type=et,
                headline=r.title, summary=(r.content or "")[:500],
                score=score, has_official=is_official, ref_raw_ids=str(r.id),
                event_time=r.published_at, created_at=dt.datetime.utcnow()
            )
            s.add(ne)
        s.commit()
