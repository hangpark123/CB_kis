...
class RawEvent(Base):
    ...
    corp_name_kr: Mapped[Optional[str]] = mapped_column(Text)
    published_at: Mapped[Optional[str]] = mapped_column(TIMESTAMP(timezone=True))
    raw_json: Mapped[Optional[dict]] = mapped_column(JSON)
    inserted_at: Mapped[Optional[str]] = mapped_column(TIMESTAMP(timezone=True))

class NormEvent(Base):
    __tablename__ = "norm_events"
    # ✅ SQLite 호환
    event_id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    stock_code: Mapped[Optional[str]] = mapped_column(VARCHAR(12))
    corp_name_kr: Mapped[Optional[str]] = mapped_column(Text)
    ...
    ref_raw_ids: Mapped[Optional[str]] = mapped_column(Text)   # CSV 문자열 보관
    event_time: Mapped[Optional[str]] = mapped_column(TIMESTAMP(timezone=True))
    created_at: Mapped[Optional[str]] = mapped_column(TIMESTAMP(timezone=True))
...
class TradingSignal(Base):
    ...
    reason: Mapped[Optional[str]] = mapped_column(Text)  # 신호 이유 (뉴스 헤드라인)
    ref_norm_event_id: Mapped[Optional[int]] = mapped_column(Integer)
...
class Trade(Base):
    ...
    executed_at: Mapped[Optional[str]] = mapped_column(TIMESTAMP(timezone=True))
    created_at: Mapped[Optional[str]] = mapped_column(TIMESTAMP(timezone=True))
...
class Position(Base):
    ...
    last_updated: Mapped[Optional[str]] = mapped_column(TIMESTAMP(timezone=True))
...
class TradingConfig(Base):
    ...
    updated_at: Mapped[Optional[str]] = mapped_column(TIMESTAMP(timezone=True))
